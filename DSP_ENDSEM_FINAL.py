...
def reconstructor(ind ,lam, y, domain, original, epoch,output_queue):
    ...
    if domain == "wavelet":
        for i in range(epoch):
            ...
            xhat = inverse_flat_wavelet_transform2(Xhat, (128,128), method)
...
def save_reconstructed_image():
    if reconstructed_image is not None:
        ...
        file_path = filedialog.asksaveasfilename(defaultextension=".png",
                                                 ...
                                                 initialfile="reconstructed_CS.png")
        if file_path:
            Image.fromarray(reconstructed_image).convert('L').save(file_path)
            messagebox.showinfo("Image Saved", f"Reconstructed image saved as {file_path}")
    else:
...
def perform_reconstruction():
    ...
    # Calculate metrics for the entire reconstructed image
    mse_segmented, psnr_segmented, ssim_segmented, ncc_segmented = calculate_metrics(original_image, reconstructed_image)
    mse_full, psnr_full, ssim_full, ncc_full = calculate_metrics(original_image, reconstructed_image_full)
...
save_button = tk.Button(root, text="Save Reconstructed Image", command=save_reconstructed_image, bg="blue", fg="white")
